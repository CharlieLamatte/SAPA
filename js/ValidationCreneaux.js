...
let tableauArchive = (function () {
    ...
    let init = function () {
        fetch('ValidationCreneaux/ReadAllCreneauxNonValide.php', {
            ...
            body: JSON.stringify({'statut_creneau': 'Invalide'}),
        })
            .then(response => {
                if (!response.ok) {
                    throw {
    ...
    function createLigne(creneau) {
        ...
        let refuser = document.createElement('button');
        refuser.className = "clickable btn btn-danger";
        refuser.textContent ='Refuser';
        refuser.id = 'refuser-button' + creneau.id_creneau;

        refuser.onclick = () => {
            updateCreneau({
                'id_creneau': creneau.id_creneau,
                'statut_creneau': 'Refuse'
            }, row);
        };
        ...
        row.append(td1, td2, td3, td4, td5, td6, td7, td8, td9, td10, td11, td12, td13, td14, td15, td16, td17);
