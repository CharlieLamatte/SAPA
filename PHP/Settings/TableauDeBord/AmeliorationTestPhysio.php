...
/**
 * Permet de récupérer le taux d'évolution du poids pour le test physique des patients au cours du temps
 */
...
if (!empty($input_data->id_territoire)) {
    ...
    $stats_all = $s->getAmeliorationTestPhysio(
        $input_data->id_territoire,
        $input_data->year,
        $input_data->id_structure,
        "-1"
    );
    $stats_hommes = $s->getAmeliorationTestPhysio(
        $input_data->id_territoire,
        $input_data->year,
        $input_data->id_structure,
        "M"
    );
    $stats_femmes = $s->getAmeliorationTestPhysio(
        $input_data->id_territoire,
        $input_data->year,
        $input_data->id_structure,
        "F"
