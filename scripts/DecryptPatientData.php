<?php
/**
 * Script qui permet de décrypter les données patients de la BDD
 * (remplace les données cryptées par une version non cryptée)
 */
...
try {
    ...
    while ($patient = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $tel_portable_patient = empty($patient['tel_portable_patient']) ? "" : EncryptionManager::decrypt(
            $patient['tel_portable_patient']
        );
        $tel_fixe_patient = empty($patient['tel_fixe_patient']) ? "" : EncryptionManager::decrypt(
            $patient['tel_fixe_patient']
        );
        $email_patient = empty($patient['email_patient']) ? "" : EncryptionManager::decrypt($patient['email_patient']);
        ...
        $nom_adresse = empty($patient['nom_adresse']) ? "" : EncryptionManager::decrypt($patient['nom_adresse']);
        $complement_adresse = empty($patient['complement_adresse']) ? "" : EncryptionManager::decrypt(
        ...
        $premier_prenom_naissance = empty($patient['premier_prenom_naissance']) ? "" : EncryptionManager::decrypt(
            $patient['premier_prenom_naissance']
        );
        $nom_naissance = empty($patient['nom_naissance']) ? "" : EncryptionManager::decrypt($patient['nom_naissance']);
        $matricule_ins = empty($patient['matricule_ins']) ? "" : EncryptionManager::decrypt($patient['matricule_ins']);
        $oid = empty($patient['oid']) ? "" : EncryptionManager::decrypt($patient['oid']);
        $code_insee_naissance = empty($patient['code_insee_naissance']) ? "" : EncryptionManager::decrypt(
            $patient['code_insee_naissance']
        );
        $nom_utilise = empty($patient['nom_utilise']) ? "" : EncryptionManager::decrypt($patient['nom_utilise']);
        $prenom_utilise = empty($patient['prenom_utilise']) ? "" : EncryptionManager::decrypt(
            $patient['prenom_utilise']
        );
        $liste_prenom_naissance = empty($patient['liste_prenom_naissance']) ? "" : EncryptionManager::decrypt(
        ...
        if ($contact_urgence) {
            $nom_contact_urgence = empty($patient['nom_contact_urgence']) ? "" : EncryptionManager::decrypt(
                $patient['nom_contact_urgence']
            );
            $prenom_contact_urgence = empty($patient['prenom_contact_urgence']) ? "" : EncryptionManager::decrypt(
                $patient['prenom_contact_urgence']
            );
            $tel_fixe_contact_urgence = empty($patient['tel_fixe_contact_urgence']) ? "" : EncryptionManager::decrypt(
                $patient['tel_fixe_contact_urgence']
            );
            $tel_portable_contact_urgence = empty($patient['tel_portable_contact_urgence']) ? "" : EncryptionManager::decrypt(
